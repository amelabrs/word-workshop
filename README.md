...
| `group` | no | Scopes the quiz's answer-choice pool to only other words sharing the same group within the tab, instead of the whole tab. Use for easily-confused words that should be drilled against each other specifically — e.g. Mother/Father/Sister/Brother share `group: "immediate"` so those questions only ever offer each other. Words without a group use the whole-tab pool (default) |
| `hidden` | no | Set `true` to pull a word out of rotation entirely — excluded from the level grid, quizzes, answer-choice pools, and Say It. The entry (and its photo) stays in `folders.js`, just inert, so it's a one-line flip to bring back later |
